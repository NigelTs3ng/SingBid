...
const AuctionCard = ({ auction, viewMode = 'grid' }) => {
  ...
  const handleCardClick = () => {
    navigate('/auction-details', { state: { auctionId: auction?.id } });
  };

  const handleQuickBid = (e) => {
    e?.stopPropagation();
    navigate('/auction-details', { state: { auctionId: auction?.id, quickBid: true } });
  };
  ...
  const getStatusBadge = () => {
    ...
    if (auction?.currentBid >= auction?.reservePrice && auction?.reservePrice > 0) {
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-right">
            ...
            <p className="text-sm font-medium text-foreground">
              S${(auction?.currentBid + auction?.bidIncrement)?.toLocaleString()}
            </p>
          </div>
        </div>
